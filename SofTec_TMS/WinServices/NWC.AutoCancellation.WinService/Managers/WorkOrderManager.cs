...
namespace NWC.AutoCancellation.WinService.Managers
{
    public class WorkOrderManager
    {
        ...
        public DescriptiveResponse<SearchResult<WorkOrderDTO>> GetWorkOrdersReadyToAutoCancel(string queryAPI_URL, string token, WorkOrderSearchCriteriaDTO searchCriteria, int retrials, int holdInterval)
        {
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    var postTask = client.PostAsJsonAsync(string.Format("GetWorkOrdersReadyToAutoCancel?retrials={0}&holdInterval={1}", retrials, holdInterval), searchCriteria);
                    postTask.Wait();
                    ...
                    if (result.IsSuccessStatusCode)
                    {
                        ...
                        return DescriptiveResponse<SearchResult<WorkOrderDTO>>.Success(returnResult.Value);
                    }
                }
        ...
        public DescriptiveResponse<Boolean> CancelWorkOrder(string commandAPI_URL, string token, EventWorkOrderDTO dto)
        {
            try
            {
                using (var client = new HttpClient())
        ...
        public DescriptiveResponse<Boolean> UpdateWorkOrderRetrials(string commandAPI_URL, string token, long workOrderID, int holdInterval)
        {
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    var postTask = client.PutAsync(string.Format("UpdateWorkOrderCancelRetrials?workOrderID={0}&holdInterval={1}", workOrderID, holdInterval), null);
                    postTask.Wait();
