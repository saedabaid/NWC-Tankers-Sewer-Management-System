...
using NWC.DriverSMS.WinService.Managers;
...
namespace NWC.DriverSMS.WinService
{
    public partial class DriverSMS : ServiceBase
    {
        ...
        protected override void OnStart(string[] args)
        {
            try
            {
                ...
                if (serviceTimer == null)
                {
                    ...
                    serviceTimer = new Timer(new TimerCallback(SendDriverSMS), null, tsInterval, tsInterval);
                }
            }
            catch (Exception ex)
            {
        ...
        public void SendDriverSMS(object state)
        {
            if (this.smsCount == 0 && this.isFinished)
            {
                try
                {
                    ...
                    LoggerManager.LogMsg(c => c.TrackingMsg(string.Format(">>>>>>>>>>>>>>>>> Start DriverSMS >>>>>>>>>>>>>>>>>")));
                    ...
                    var resultDriverSMSs = new DescriptiveResponse<SearchResult<DriverSMSDTO>>();

                    if (userDTO.Value != null && !string.IsNullOrEmpty(userDTO.Value.Token))
                    {
                        var searchCriteria = new DriverSMSSearchCriteria()
                        {
                            StatusIDs = new List<int>() { 1 },
                            PageFilter = new PageFilter() { PageSize = this.take, PageIndex = 1 }
                        };

                        resultDriverSMSs = smsManager.GetDriverSMSs(this.queryAPI_URL, userDTO.Value.Token, searchCriteria).Result;

                        if (resultDriverSMSs.Value != null && resultDriverSMSs.Value.Result != null)
                        {
                            this.smsCount = resultDriverSMSs.Value.TotalCount;

                            LoggerManager.LogMsg(c => c.TrackingMsg(string.Format("DriverSMS: Total SMS Count: {0}", this.smsCount)));

                            foreach (var sms in resultDriverSMSs.Value.Result)
                            {
                                var isSentSMS = smsManager.SendDriverSMS(sms.ID, sms.DriverMobileNo, sms.OrderNumber, sms.SMSText, this.smsLang, this.smsType, this.eventSource, this.eventPriority);
                                ...
                                LoggerManager.LogMsg(c => c.TrackingMsg(string.Format("DriverSMS: ID: {0}, Order Number: {1}, SMS Is Sent: {2}", sms.ID, sms.OrderNumber, isSentSMS)));
