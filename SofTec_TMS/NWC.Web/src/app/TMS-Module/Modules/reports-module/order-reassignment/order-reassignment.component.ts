...
import { OrderReassignmentReportSC } from "src/app/TMS-Module/Models/search-criteria/order-reassignment.model";
import { SearchResult } from "src/app/TMS-Module/Models/common/search-result";
import { OrderReassignment } from "src/app/TMS-Module/Models/order-reassignment.model";
...
@Component({
  selector: "app-order-reassignment",
  templateUrl: "./order-reassignment.component.html",
  styleUrls: ["./order-reassignment.component.scss"],
})
export class OrderReassignmentComponent implements OnInit {
  advancedDiv = false;
  pagePermission: string = "";
  tableLoading = false;

  SearchCriteria: OrderReassignmentReportSC;
  searchResult = new SearchResult<OrderReassignment>();
  IsArabic = false;
  ...
  load() {
    ...
    this.titleService.setTitle(
      this.translateService.instant("OrderReassignmentReport")
    );
  }
  ...
  setDefaultSearchValues() {
    this.SearchCriteria = new OrderReassignmentReportSC();
  ...
  searchCaller() {
    ...
    this.gateService.GetOrderReassignmentReport(modifiedCriteria).subscribe(
      (res) => {
        if ((res || ({} as any)).Value) {
  ...
  onexportOrders() {
    ...
    this.gateService.GetOrderReassignmentReport(clonedObj).subscribe(
      (res) => {
        if (!res || res.IsErrorState == false) {
          ...
          let excelJson = res.Value.Result.map((value) => {
            let r = {
              ...
              OldTankerNumber: value.OldTankerCode,
              OldDriver: value.OldDriver,
              ReassignmentDate: value.ReassignmentDate,
              NewTankerNumber: value.NewTankerCode,
              NewDriver: value.NewDriver,
