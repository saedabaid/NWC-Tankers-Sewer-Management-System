...
import { Report_SoqyaScheduledPerDay } from 'src/app/TMS-Module/Models/report-soqya-scheduled-per-day.model';
import { ReportScheduledPerDaySC } from 'src/app/TMS-Module/Models/search-criteria/report-scheduled-per-day-SC.model';
...
@Component({
  selector: 'app-soqya-scheduled-per-day',
  templateUrl: './soqya-scheduled-per-day.component.html',
  styleUrls: ['./soqya-scheduled-per-day.component.scss']
})
export class SoqyaScheduledPerDayComponent implements OnInit {

  SearchCriteria: ReportScheduledPerDaySC;

  selectMenuOptions2 = {
    singleSelect: true,
    enableSearchFilter: true,
  };
  searchResult: Report_SoqyaScheduledPerDay[] = [];
  ...
  loadDDLsGV() {
    this.titleService.setTitle(this._translate.instant("ReportSoqyaScheduledPerDay"));
  ...
  isValidModel(): boolean {
    ...
    if (!this.SearchCriteria.SelectedDate) {
      validationMessages.push("DateRequired");
    }
  ...
  searchCaller() {
    ...
    this.reportService.GetSoqyaSchedulePerDay(clonedObj).subscribe(
      res => {
        if (res.Value != null) {
  ...
  onexportOrders() {
    ...
    this.reportService.GetSoqyaSchedulePerDay(clonedObj).subscribe(res => {
      if (res.IsErrorState == false) {
        if (
          isNullOrUndefined(res.Value) ||
          res.Value.length == 0
        ) {
        ...
        let excelJson = res.Value.map(value => {
          let r = {
            Day: this._DateUtilityService.ConvertDateToExcel(value.DayOfMonth, false),
            DayOfWeek: this._DateUtilityService.getDayOfWeek(value.DayOfMonth),
            TotalSoqyaCounts: value.TotalCounts,
            TotalQuantityM3: value.SumQuantities
          }

          value.CapacityList.forEach(element => {
            r[element.Quantity.toString()] = element.SchedulesCount;
          });

          return r;
        });

        this._ExcelService.exportAsExcelFile(excelJson, this._translate.instant("ReportSoqyaScheduledPerDay"));
      }
    }
      , err => {
  ...
  cloneSearchCriteriaObjectWithoutTimeZone() {
    ...
    if (this.SearchCriteria.SelectedDate) {
      clonedObj.SelectedDate = new Date(this.SearchCriteria.SelectedDate.getTime());
      clonedObj.SelectedDate.setMinutes(
        clonedObj.SelectedDate.getMinutes() - clonedObj.SelectedDate.getTimezoneOffset());
    }
