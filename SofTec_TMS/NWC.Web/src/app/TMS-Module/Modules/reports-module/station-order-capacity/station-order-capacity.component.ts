...
@Component({
  selector: 'app-station-order-capacity',
  templateUrl: './station-order-capacity.component.html',
  styleUrls: ['./station-order-capacity.component.scss']
})
export class StationOrderCapacityComponent implements OnInit {
  ...
  loadDDLsGV() {
    this.titleService.setTitle(this._translate.instant("ReportStationOrderCapacity"));
  ...
  searchCaller() {
    ...
    this.reportService.GetStationOrderCapacity(clonedObj).subscribe(
      res => {
        if (res.Value != null) {
  ...
  onexportOrders() {
    ...
    this.reportService.GetStationOrderCapacity(clonedObj).subscribe(res => {
      if (res.IsErrorState == false) {
        ...
        this._ExcelService.exportAsExcelFile(excelJson, this._translate.instant("ReportStationOrderCapacity"));
      }
    }
      , err => {
