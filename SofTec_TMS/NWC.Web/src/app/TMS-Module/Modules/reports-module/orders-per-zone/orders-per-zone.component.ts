...
@Component({
  ...
  templateUrl: './orders-per-zone.component.html',
  styleUrls: ['./orders-per-zone.component.scss']
})
export class OrdersPerZoneComponent implements OnInit {
  ...
  searchCaller() {
    ...
    this.reportService.GetOrdersPerZone(clonedObj).subscribe(
      res => {
        if (res.Value != null) {
  ...
  onexportOrders() {
    ...
    this.reportService.GetOrdersPerZone(clonedObj).subscribe(res => {
