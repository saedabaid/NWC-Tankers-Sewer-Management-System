...
import { isNullOrUndefined, minutesToHHMM } from 'src/app/shared/utilities/utilities';
...
import { Report_OrderPerZone } from 'src/app/TMS-Module/Models/report-order-per-zone.model';
...
@Component({
  selector: 'app-station-service-time',
  templateUrl: './station-service-time.component.html',
  styleUrls: ['./station-service-time.component.scss']
})
export class StationServiceTimeComponent implements OnInit {

  SearchCriteria: ReportSC;

  selectMenuOptions2 = {
    //singleSelect: true
  };
  searchResult = new SearchResult<Report_OrderPerZone>();
  ...
  loadDDLsGV() {
    this.titleService.setTitle(this._translate.instant("ReportStationServiceTime"));
  ...
  setDefaultSearchValues() {
    this.SearchCriteria = new ReportSC();
  ...
  searchCaller() {
    ...
    this.reportService.GetStationServiceTime(clonedObj).subscribe(
      res => {
        if (res.Value != null) {
  ...
  onexportOrders() {
    ...
    this.reportService.GetStationServiceTime(clonedObj).subscribe(res => {
      if (res.IsErrorState == false) {
        ...
        let excelJson = res.Value.Result.map(value => {
          let r = {
            ...
            TotalOrdersCount: value.TotalNoOfOrders,
            TotalQuantityM3: value.TotalQuantity,

            AvgCreateToDeliveredTime: minutesToHHMM(value.AvgCreateToDeliveredTime),
            AvgCreateToOutTime: minutesToHHMM(value.AvgCreateToOutTime),
            AvgOutToDeliveredTime: minutesToHHMM(value.AvgOutToDeliveredTime)
          }
          return r;
        });

        this._ExcelService.exportAsExcelFile(excelJson, this._translate.instant("ReportStationServiceTime"));
      }
    }
      , err => {
