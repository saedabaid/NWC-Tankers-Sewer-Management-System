...
@Component({
  selector: "app-orders-day-hours-chart",
  templateUrl: "./orders-day-hours-chart.component.html",
  styleUrls: ["./orders-day-hours-chart.component.scss"]
})
export class OrdersDayHoursChartComponent implements OnInit, OnDestroy {

  searchSubscriper: Subscription;
  loading_DayHoursChart = false;
  ...
  loadChart(filters: DashboardSC) {
    ...
    this.loading_DayHoursChart = true;
    this.dashboardService.GetOrdersCountGroupByDayHours(filters).subscribe(res => {

      if( !res.IsErrorState && res.Value){

        let chart = am4core.create("dayHoursChart", am4charts.XYChart);
    ...
    , err => {
      this.loading_DayHoursChart = false;
    }, () => {
      this.loading_DayHoursChart = false;
    });
